...
```
otice that here we used `total` and `population` inside the function, which are objects that are **not** defined in our workspace. But why don’t we get an error?
...
data(murders)
my_states <- murders %>%
  mutate(murders, rate=total/population*10000, rank=rank(-rate)) %>%
  filter(region %in% c("Norteast", "West") & rate<1) %>%
  select(state, rate, rank)
my_states
```
