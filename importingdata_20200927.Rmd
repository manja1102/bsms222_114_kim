...
The <em>path</em> of a file is a list of directory names that can be thought of as instructions on what folders to click on, and in what order, to find the file. If these instructions are for finding the file from the root directory we refer to it as the <em>full path</em>. If the instructions are for finding the file starting in the working directory we refer to it as a <em>relative path</em>. Section [38.3](https://rafalab.github.io/dsbook/unix.html#filesystem) provides more details on this topic.
...
|:--------:|:------------------------------:|:------------:|
|read_table|white space sperated values     |txt           |
...
The Microsoft Excel formats permit you to have more than one spreadsheet in one file. These are referred to as <em>sheets</em>. The functions listed above read the first sheet by default, but we can also read the others. The `excel_sheets` function gives us the names of all the sheets in an Excel file. These names can then be passed to the `sheet` argument in the three functions above to read sheets other than the first.
...
```{r}
read_csv(files)
a <- file.path(path, "2010_bigfive_regents.xls")
read_xls(a)
b <- file.path(path,"carbon_emissions.csv")
read_csv(b)
c <- file.path(path,"fertility-two-countries-example.csv")
read_csv(c)
d <- file.path(path,"HRlist2.txt" )
read_csv(d)
e <- file.path(path,"life-expectancy-and-fertility-two-countries-example.csv")
read_csv(e)
f <- file.path(path,"murders.csv")
read_csv(f)
g <- file.path(path, "olive.csv")
read_csv(g)
h <- file.path(path, "RD-Mortality-Report_2015-18-180531.pdf")
read_csv(h)
i <- file.path(path,"ssa-death-probability.csv")
read_csv(i)
```
...
When reading in spreadsheets many things can go wrong. The file might have a multiline header, be missing cells, or it might use an unexpected encoding[^17]. We recommend you read this post about common issues found here: https://www.joelonsoftware.com/2003/10/08/the-absolute-minimum-every-software-developer-absolutely-positively-must-know-about-unicode-and-character-sets-no-excuses/.
